...
const searchComponentChange = (
  ...
  for (const component of components) {
    if (component.type === "section" || component.type === "page") {
      const newContents = searchComponentChange(
        component.contents || [],
        id,
        callback
      );
...
export const metaflow_card_update: (
  data: Record<string, types.CardComponent>
) => boolean = (data) => {
  ...
  cardData.update((value: types.CardResponse) => {
    ...
    for (const componentId in updateData) {
      value.components = searchComponentChange(
        value.components || [],
        componentId,
        (_) => {
